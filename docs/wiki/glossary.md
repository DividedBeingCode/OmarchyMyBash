...
| **Look** | A named appearance bundle: a `config_set`-shaped patch plus a palette directive (`keep` / `theme` / curated palette key). Twenty-eight curated Looks are compiled into the daemon, each tagged `structure` (respects your palette) or `complete` (brings its own); user Looks live in `[looks.<name>]` in `config.toml` and shadow curated names. Applied via `looks_apply` (transient try mode reverts with `reload_config`) and browsed in the Studio's Looks tab. |
