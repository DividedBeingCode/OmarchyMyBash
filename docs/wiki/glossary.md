...
| **Look** | A complete appearance definition: colors, style preset, separators, frame, glyphs, prompt characters and line count. Applying one resets every key in `looks::LOOK_OWNED` before merging the Look's patch, so the result is the Look itself and never a blend with the previously applied one. It does not touch which segments are enabled — that is a standing user choice. 52 curated Looks are compiled into the daemon; user Looks live in `[looks.<name>]` in `config.toml` and shadow curated names. Applied via `looks_apply` (transient try mode reverts with `reload_config`) and browsed in the Studio's Looks tab. See also **desktop lock**. |
| **Desktop lock** | `[theme] follow_desktop`. Binds the prompt's colors to the Omarchy desktop theme so that applying a Look changes its shape, glyphs and framing but leaves the palette alone. Deliberately outside `LOOK_OWNED`, so no Look can clear it. |
...
| **`O10K_TERM`** | The terminal the shell is in, resolved once per shell by the adapter (env, else an XTVERSION probe) and sent to the daemon over the env channel. Also the manual override. `unknown` is a real value meaning "conservative profile". See [Bash Adapter](bash-adapter.md). |
| **XTVERSION** | `CSI > q`. A terminal answers with its name and version. The only reliable way to identify foot, which unsets `TERM_PROGRAM` and (under Omarchy) reports `TERM=xterm-256color`. |
