...
| `look` | string | No | `""` | **v0.5.** Dry-run render a Look: resolves the named Look (user entries from `[looks.<name>]` shadow curated ones), applies it with `apply_look` — the same atomic apply the Apply button uses, which clears the Look-owned keys before merging the patch, so a card shows exactly what applying would produce — and renders the result. Nothing is persisted; an unknown look name silently falls back to the current config. |
